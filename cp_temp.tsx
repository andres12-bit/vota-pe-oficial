        ...
        <div className="text-center group">
            <div className="relative w-20 h-20 mx-auto mb-2">
                <svg className="w-full h-full -rotate-90" viewBox="0 0 36 36">
                    <circle cx="18" cy="18" r="15.9" fill="none" stroke="rgba(0,0,0,0.06)" strokeWidth="2.5" />
            ...
            <div className="text-[10px] font-bold tracking-[1.5px] uppercase" style={{ color: '#64748b' }}>{label}</div>
        ...
        <div className="min-h-screen" style={{ background: 'transparent' }}>
            ...
            <div className="cp-hero-banner">
                <div className="cp-hero-inner">
                    <button onClick={() => router.back()} className="cp-back-btn">
                    ...
                    <div className="cp-hero-content">
                        ...
                        <div className="cp-hero-photo-wrap">
                            <img
                                src={getCandidatePhoto(candidate.photo, candidate.name, 160, candidate.party_color)}
                                onError={(e) => { const fb = getPhotoFallback((e.target as HTMLImageElement).src); if (fb && !(e.target as HTMLImageElement).dataset.retried) { (e.target as HTMLImageElement).dataset.retried = '1'; (e.target as HTMLImageElement).src = fb; } else { (e.target as HTMLImageElement).src = getAvatarUrl(candidate.name, 160, candidate.party_color); } }}
                                alt={candidate.name}
                                width={160} height={160}
                                className="cp-hero-photo"
                            />
                            <div className="cp-hero-photo-ring" style={{ borderColor: candidate.party_color }} />
                        </div>

                        {/* Info */}
                        <div className="cp-hero-info">
                            <h1 className="cp-hero-name">{candidate.name}</h1>
                            <div className="cp-hero-badges">
                                <Link href={`/party/${candidate.party_id}`} className="cp-badge-party" style={{ background: `${candidate.party_color}22`, color: candidate.party_color, borderColor: `${candidate.party_color}55` }}>
                                    {candidate.party_logo && <img src={candidate.party_logo} alt="" className="cp-badge-logo" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />}
                                    {candidate.party_abbreviation} • {candidate.party_name}
                                </Link>
                                <span className="cp-badge-position">{positionLabels[candidate.position] || candidate.position}</span>
                                {sectorData?.analysis && (() => {
                                    ...
                                    return <span className="cp-badge-ai" onClick={() => setShowAiModal(true)}>🤖 IA: {aiPct}%</span>;
                                })()}
                            </div>
                            <div className="cp-hero-meta">
                                <span className="cp-meta-region">📍 {candidate.region}</span>
                                <MomentumIndicator score={momScore} />
                            </div>
                            {candidate.biography && <p className="cp-hero-bio">{candidate.biography}</p>}
                        </div>
                            ...
                            <div className="cp-hero-score-circle">
                                <svg viewBox="0 0 120 120" className="cp-hero-score-svg">
                                    ...
                                    <circle cx="60" cy="60" r="52" fill="none" stroke="#fff" strokeWidth="8"
                                        strokeDasharray={`${(finalScore / 100) * 327} 327`} strokeLinecap="round"
                                        style={{ transform: 'rotate(-90deg)', transformOrigin: 'center', transition: 'stroke-dasharray 1.5s ease-out' }} />
                                </svg>
                                <div className="cp-hero-score-value">{finalScore.toFixed(1)}</div>
                            </div>
                            <div className="cp-hero-score-label">SCORE FINAL</div>
                            <div className="cp-hero-score-stars">
                                <StarRating rating={starsVal} />
                                <span>{starsVal.toFixed(1)}/5</span>
                            </div>
                            <div className="cp-hero-votes">🗳️ {candidate.vote_count?.toLocaleString() || '0'} votos</div>
                            <button
                                onClick={() => addCandidate(candidate)}
                                disabled={isInCart(candidate.id)}
                                className="cp-hero-select-btn"
                                style={isInCart(candidate.id) ? { background: 'rgba(255,255,255,0.15)', cursor: 'default' } : {}}>
            ...
            {/* ═══ SCORE SUMMARY CARDS ═══ */}
            <div className="cp-score-cards">
                <div className="cp-score-card" style={{ borderTopColor: '#312e81' }}>
                    <div className="cp-score-card-icon" style={{ background: '#312e8115', color: '#312e81' }}>📄</div>
                    <div className="cp-score-card-label">Hoja de Vida</div>
                    <div className="cp-score-card-value" style={{ color: '#312e81' }}>{hojaScore.toFixed(0)}</div>
                    <div className="cp-score-card-weight">30% del score</div>
                    <div className="cp-score-card-bar"><div style={{ width: `${hojaScore}%`, background: '#312e81' }} /></div>
                </div>
                <div className="cp-score-card" style={{ borderTopColor: '#1e3a5f' }}>
                    <div className="cp-score-card-icon" style={{ background: '#1e3a5f15', color: '#1e3a5f' }}>📋</div>
                    <div className="cp-score-card-label">Plan de Gobierno</div>
                    <div className="cp-score-card-value" style={{ color: '#1e3a5f' }}>{planScore.toFixed(0)}</div>
                    <div className="cp-score-card-weight">30% del score</div>
                    <div className="cp-score-card-bar"><div style={{ width: `${planScore}%`, background: '#1e3a5f' }} /></div>
                </div>
                <div className="cp-score-card" style={{ borderTopColor: '#0e7490' }}>
                    <div className="cp-score-card-icon" style={{ background: '#0e749015', color: '#0e7490' }}>💼</div>
                    <div className="cp-score-card-label">Experiencia</div>
                    <div className="cp-score-card-value" style={{ color: '#0e7490' }}>{experienceScore.toFixed(0)}</div>
                    <div className="cp-score-card-weight">25% del score</div>
                    <div className="cp-score-card-bar"><div style={{ width: `${experienceScore}%`, background: '#0e7490' }} /></div>
                </div>
                <div className="cp-score-card" style={{ borderTopColor: '#2e7d32' }}>
                    <div className="cp-score-card-icon" style={{ background: '#2e7d3215', color: '#2e7d32' }}>🛡️</div>
                    <div className="cp-score-card-label">Integridad</div>
                    <div className="cp-score-card-value" style={{ color: '#2e7d32' }}>{integrScore.toFixed(0)}</div>
                    <div className="cp-score-card-weight">15% del score</div>
                    <div className="cp-score-card-bar"><div style={{ width: `${integrScore}%`, background: '#2e7d32' }} /></div>
                </div>
            </div>
                                ...
                                const vpContent = (
                                    <div key={vp.id} className="flex items-center gap-3 px-4 py-3 rounded-xl flex-1 min-w-[200px] cursor-pointer hover:scale-[1.01] transition-all" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--vp-border)' }}>
                                        <img
                                            ...
                                            onError={(e) => { const fb = getPhotoFallback((e.target as HTMLImageElement).src); if (fb && !(e.target as HTMLImageElement).dataset.retried) { (e.target as HTMLImageElement).dataset.retried = '1'; (e.target as HTMLImageElement).src = fb; } else { (e.target as HTMLImageElement).src = getAvatarUrl(vp.name, 48, candidate.party_color); } }}
                                                    ...
                                                    {Object.entries(signalLabels).map(([key, meta]) => {
                                                        ...
                                                        return (
                                                            <div key={key} className="p-3 rounded-xl" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid var(--vp-border)' }}>
                                                                ...
                                                                <div className="w-full h-1.5 rounded-full mb-1" style={{ background: '#f1f5f9' }}>
